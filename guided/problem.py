...
class DoublePendulumProblem(object):
    def __init__(self, params=None, time_range=None):
        self.time_range = time_range or np.arange(0.0, 10, 0.01)
    ...
    def compute_trajectory(self, p, initial_state):
        # simulation and visualization
        y = p.simulate(initial_state, self.time_range, visualize=False)
        u = [p.controller(y[i,:], self.time_range[i]) for i in range(y.shape[0])]
        u = np.vstack(u)
        return (y, u)
    ...
    def cost(self, trajectory):
        ...
        total_cost = position_diff * self.exp_time(x.shape[0])
    ...
    def animate_trajectory(self, plant, trajectory):
        x, _ = trajectory
        return plant.animation(x, self.time_range)
